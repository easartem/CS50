...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            if (preferences[i][0] == i & candidates[j].eliminated == false)
            {
                candidates[j].votes++;
            }
            else if (preferences[i][1] == i & candidates[j].eliminated == true)
        }
    }
    return;
}
